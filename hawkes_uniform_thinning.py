...
# multivariate Hawkes process with uniform thinning -- simulation and Bayesian inference
# adapted from Richard Kwo - May, 2014
# Emily Mu - Oct, 2018
...
# class begin
class Hawkes_Uniform_Thinning(object):
    """docstring for Hawkes"""
    def __init__(self, A=2, C=2, T=100.0, non_diagonal=False):
        '''
        Hawkes object
        A: number of agents
        T: max time 
        ...
        self._baseline_rate = zeros(self._C)
    ...
    def set_nu(self, nu, x=None, y=None):
        ...
        When setting the whole self._nu to an C x C matrix nu, passing x = None and y = None
        nu[p, q] is the excitation height from p to q
        '''
        if all(self._nu[x,y] == nu): return  # avoid reevaluating ll when not changed
        if x is None and y is None:
            assert shape(nu) == (self._C, self._C) and all(nu>=0), '%s' % (nu)
    ...
    def set_baseline_rate(self, baseline_rate, x=None):
        '''set the baseline_rate (lambda_0), of shape(C)'''
        if all(self._baseline_rate[x] == baseline_rate): return  # avoid reevaluating ll when not changed
        if x is None:
            assert shape(baseline_rate) == (self._C, ) and all(baseline_rate>0), '%s' % (baseline_rate)
        else:
    ...
    def plot_data_events(self, show_figure=True):
        ...
        for x in range(self._A):
            ...
            ax.vlines(x=self._agent_data[x], ymin=0, ymax=1)
            ax.axvline(x=self._time_to_split, ls='--', color='r')
            ax.set_title("agent %d (%d events)" % (x+1, len(self._agent_data[x])))
    ...
    def loadData(self, agent_data, time_to_split=100.0):
        '''
        ...
        '''
        assert len(agent_data)==self._A
        assert time_to_split>0 and time_to_split <= self._T
        #compile agent data to data based on cluster assignments
        data = [[] for x in range(self._C)]
        for i in range(self._A):
            data[self._clusters_assign[i]] += agent_data[i]
        for i in range(self._C):
            data[i] = sorted(data[i])
        print("Data = ", data)
        print("Agent Data = ", agent_data)
        self._time_to_split = time_to_split
        self._train_set_sizes = [0 for x in range(self._C)]
        self._test_set_sizes = [0 for x in range(self._C)]
        for x in range(self._C):
        ...
        for x in range(self._C):
            print("-- %d events for cluster %d  (train: %4d, test: %4d)" % (len(self._data[x]), x+1, self._train_set_sizes[x], self._test_set_sizes[x]))
...
def unitest_simulation():
    ...
    print("max time T = ", T)
    print("C = ", C)
    hawkes_proc = Hawkes_Uniform_Thinning(A=A, T=T, C=C)

    # parameters
    nu = array([[0, 0.5], [0.5, 0]])
    time_decay = array([0.4, 0.8])
    baseline_rate = array([0.5, 0.5])
    cluster_assign = array([0, 1, 0, 1])
    ...
    data, agent_data = hawkes_proc.simulate()
    print("Data = ", data)
    print("Agent Data = ", agent_data)
    hawkes_proc.loadData(agent_data)
